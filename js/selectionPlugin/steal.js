(function () {
     ...
     */ {
        ...
        joinFrom: function (url, expand) {
            ...
            if (this.protocol()) { //if we are absolutely referenced
                ...
                if (firstDomain && firstDomain == secondDomain) {
                    ...
                    return firstDomain ? this.afterDomain() : this.toReferenceFromSameDomain(url);
                } else {
                    return this.path;
                }
        ...
        normalize: function () {
            ...
            if (/^\/\//.test(this.path)) { //if path is rooted from steal's root (DEPRECATED) 
                ...
                this.path = "./" + this.path;
            } else if (/^[^\.|\/]/.test(this.path)) {} else {
                if (this.relative() || File.cur().isCrossDomain() && //if current file is on another domain and
    ...
    steal.p = {
        ...
        init: function (options) {
            ...
            //handle callback functions	
                ...
                this.options = steal.makeOptions(extend({}, typeof options == 'string' ? {
                    src: options
                } : options));
    ...
    extend(steal, {
         ...
         */
        then: function () {
            var args = typeof arguments[0] == 'function' ? arguments : [function () {}].concat(makeArray(arguments))
    ...
    steal.p.load = after(steal.p.load, function (stel) {
        if (win.document && !this.completed && !this.completeTimeout && (this.options.protocol == "file:" || !support.error)) {
            var self = this;
    ...
    interactives = {},
        getInteractiveScript = function () {
            var i, script, scripts = doc[STR_GET_BY_TAG]('script');
